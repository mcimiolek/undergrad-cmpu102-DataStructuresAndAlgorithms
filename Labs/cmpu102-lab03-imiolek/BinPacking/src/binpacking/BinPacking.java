...
public class BinPacking extends GraphicsProgram {
  ...
  private static int max_disks = 10;
  ...
  private static int disksPlaced = 0;
    ...
    private static Disk[] expandedDiskArray(Disk[] oldDiskArray, Random randomizer) {
        Disk[] diskArray = new Disk[max_disks * 2];
        for (int diskNumber = 0; diskNumber < max_disks; diskNumber++) {
            diskArray[diskNumber] = oldDiskArray[diskNumber];
        }
        for (int diskNumber = max_disks; diskNumber < diskArray.length; diskNumber++) {
            diskArray[diskNumber] = new Disk(HOME_X, HOME_Y, randomRadius(randomizer), randomColor(randomizer));
        }
        
        max_disks = max_disks*2;
        return diskArray;
    }
